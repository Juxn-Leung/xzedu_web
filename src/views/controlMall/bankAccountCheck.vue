...
<template>
  <div class="trainlist">
    ...
    <div class="table_content">
      <Row>
        <Table :columns="columns" :data="data" ref="table" :loading="loading" :height="tableHeight">
          <template slot="action" slot-scope="{ row, index }">
            <div v-if="row.status == 1" class="yitong" type="text">已通过</div>
            <div v-if="row.status == 2" class="yitong" type="text">不通过</div>
    ...
    <p
      ...
      >
        ...
        <div slot="footer">
          <Button type="primary" :loading="modal_loading" @click="submitAudit">确 定</Button>
        </div>
      </Modal>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    ceshi() {
      ...
      console.log("测试"+computed.menuBtns);
    },
    ...
    getCourseList() {
      this.searchForm.type = this.num;
      accCheckList(this.searchForm).then((res) => {
