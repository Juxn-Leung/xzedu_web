...
<div class="trainlist">
    ...
    <Row>
        <div class="search" v-if="seach_panel">
            <Form :model="searchForm" ref="searchForm" style="display: flex; float: left" :label-width="80">
                <FormItem label="省市区" v-if="showAddInput == 3">
                    <Cascader change-on-select @on-visible-change="clickAreaSearch" ref="cascader" :data="addressDataSearch" :value="areaId" @on-change="changeAreaSearch" :load-data="loadDataSearch" transfer></Cascader>
                </FormItem>
                ...
                <FormItem label="审核状态" v-if="showAddInput == 3">
                    <Select clearable v-model="searchForm.status" :value="searchForm.status" transfer style="width: 100px" @on-change="changeCondition">
                ...
                <FormItem label="登记时间">
                    <Row>
                        <DatePicker v-model="searchForm.findTime" :value="searchForm.findTime" format="yyyy年MM月dd日" type="date" placeholder="选择日期" style="width: 150px; z-index: 7" @on-change="changeDate"></DatePicker>
                    </Row>
                </FormItem>

                <FormItem label="关键字" style="width: 400px">
                    <Input v-model="searchForm.key" placeholder="请输入机构名称、办学许可、营业执照进行搜索" @input="keyInput" />
                </FormItem>
            </Form>
    ...
    <div v-if="addlenght == 1" class="table_content">
        <Row>
            <Table size="large" :columns="columns" :data="data" ref="table" :loading="loading" :height="tableHeight" @on-selection-change="changeSelect">
                <template slot="actionx" slot-scope="{ row }">
                    <div v-if="genre == 2">
                        <Button v-if="row.status == 0" type="primary" size="small" style="background: #edf3ff; color: #437ef6; cursor: auto">待审核</Button>
                        <Button v-if="row.status == 1" type="text" size="small" style="background: #e5fbe5; color: #3a955d; cursor: auto">已通过</Button>
                        <Button v-if="row.status == 2" type="primary" size="small" style="background: #ffeaea; color: #ff7d7d; cursor: auto">未通过</Button>
                    </div>
                </template>

                <template slot="action" slot-scope="{ row, index }">
                    <div v-if="seach_panel == true">
                        <Button type="primary" size="small" style="margin-right: 6px; background: #427ef4" @click="handleEdit(row, index)">查看</Button>

                        <Button v-if="row.status == 0" @click="succes(row, index)" size="small" style="margin-right: 6px; margin-top: 6px; margin-bottom: 6px">通过</Button>
                        <Button v-if="row.status == 0" @click="erno(row, index)" class="no" size="small" style="margin-right: 6px; margin-top: 6px; margin-bottom: 6px">不通过</Button>
                    </div>
                </template>
            </Table>
        </Row>
        ...
        <Row type="flex" justify="end" class="page">
            <div ref="footer" class="footer">
                <Page :current="searchForm.pageNumber" :total="total" @on-change="changePage" @on-page-size-change="changePageSize" :page-size-opts="[10, 20, 50]" show-total show-elevator show-sizer></Page>
            </div>
        </Row>
    </div>
    ...
    <Modal v-model="modal">
        ...
        <p slot="close" style="
          ...
          background: white;
        ">
            <Icon @click.native="addceshi" type="ios-close-circle" style="margin-top: 1px; margin-left: 0px" size="24"></Icon>
        </p>
        ...
        <div v-if="status == 2" class="tex">
            <Form :disabled="disabled" :model="formLeft" label-position="left" :label-width="100">
                <FormItem label="备注内容">
                    <Input type="textarea" v-model="formLeft.audit" :rows="4" placeholder="请输入审核不通过的原因" />
                </FormItem>
            </Form>
        </div>
        <div slot="footer">
            <Button v-if="stats == 1" type="primary" @click="submitAuditx">确 定</Button>
            <Button v-else type="primary" @click="submitAudit">确 定</Button>
        </div>
    </Modal>

    <Modal title="View Image" width="670" :styles="{ top: '15%' }" v-model="visible">
        <p slot="header" style="
          color: #606060;
          text-align: center;
          font-weight: 550;
...
export default {
    ...
    methods: {
        ...
        getTableInfo() {
            ...
            correctlist(this.searchForm).then((res) => {
                console.log(res);
                if (res.code == 200) {
