        ...
        >
          ...
          <FormItem label="关键字">
            <Input
              ...
              @input="selectInput"
              placeholder="请输入账户名称,开户银行，银行账号查询"
              style="width: 300px"
              clearable
            />
          </FormItem>
...
export default {
  ...
  methods: {
    ...
    submitAudit() {
      ...
      whiteinvoiceopen(params).then((res) => {
        console.log(res);
        if (res.code == 200) {
    ...
    table(index) {
      ...
      this.searchForm.status = this.num;
    ...
    // 获取数据
    getCourseList() {
      whiteinvoiceList(this.searchForm).then((res) => {
