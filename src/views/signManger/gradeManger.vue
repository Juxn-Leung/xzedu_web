      ...
      <div class="search" v-if="seach_panel">
        ...
        >
          ...
          <FormItem label="班级名称" style="width:360px;">
            <Input v-model="searchForm.key" placeholder="请输入班级名称关键字" @input="keyInput" />
          </FormItem>
        </Form>
      ...
      >
        <template slot="action" slot-scope="{ row, index }">
          ...
          <Button
            ...
            size="small"
            style="margin-right: 6px;"
            @click="gradeparticulars(row, index)"
          >详情</Button>

          <Button type="error" size="small" @click="collectExport(row, index)">导出</Button>
        </template>
      </Table>
    </Row>
    <!-- 分页 -->
...
export default {
  ...
  methods: {
    getUserInfo() {
      ...
      this.searchForm.schoolId = this.$route.params.id;
    ...
    classStatList() {
      ...
      classStatList(this.searchForm).then((res) => {
        console.log(res);
        if (res.code == 200) {
    ...
    gradeparticulars(row, index) {
      ...
      this.$router.push({
        name: "gradeSignature",
        params: { schoolId: schoolId, type: type, className: className },
      });
    },
    ...
    collectExport(row, index) {
      ...
      if (state) {
        link.href =
          `/HealthReport-admin/admin/collect/export?schoolId=` +
          schoolId +
          `&type=` +
          type +
          `&className=` +
