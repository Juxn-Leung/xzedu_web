    ...
    <Row>
      <div class="search" v-if="seach_panel">
        ...
        >
          ...
          <FormItem label="学校名称" style="width:360px;">
            <Input v-model="searchForm.key" placeholder="请输入学校名称关键字" @input="keyInput" />
          </FormItem>
        </Form>
 <div v-if="ex =='1'"   style="float: right; margin: 11px 15px;">
          <Button type="success" @click="derive">导出</Button>
        </div>
    ...
    <Modal v-model="modalStatus" :title="modalTitle" :closable="false" :mask-closable="false">
      ...
      >
        <FormItem label="老师头像">
          <div class="demo-upload-list" v-for="(item,index) in avatarList" :key="index">
            <template v-if="item !== ''">
              ...
              />
              <Icon class="eyeIcon" type="ios-eye-outline" @click.native="eyeIcon(item)"></Icon>
            </template>
          </div>
          <Upload
        ...
        <FormItem label="任教年级" prop="gradeIds">
          <Select v-model="formItem.gradeIds" multiple @on-change="changeGrade">
            <Option
        ...
        <FormItem label="任教科目" prop="subjectId">
          <Select v-model="formItem.subjectId" @on-change="changeSubject">
            <Option
...
export default {
  ...
  methods: {
    ...
    schStatList() {
      ...
      schStatList(this.searchForm).then((res) => {
        console.log(res);
        if (res.code == 200) {
    ...
    collectExport(row, index) {
      ...
      if (state) {
        link.href =
          `/HealthReport-admin/admin/collect/export?schoolId=` + schoolId + `&type=` + type;
      } else {
        link.href =
          `/admin/collect/export?schoolId=` + schoolId + `&type=` + type;
      }
    ...
    //导出全部
    derive(){
     let areaId =  this.searchForm.areaId;
      ...
      if (state) {
        link.href =
          `/HealthReport-admin/admin/collect/exportByAreaId?areaId=` + areaId;
      } else {
        link.href =
          `/admin/collect/exportByAreaId?areaId=` + areaId;
      }
    ...
    handlesee(row, index) {
      ...
      let id = row.id;
      console.log(id);
      this.$router.push({ name: "gradeList", params: { id: id } });
    },
