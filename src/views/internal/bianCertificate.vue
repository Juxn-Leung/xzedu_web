    ...
    <div class="tab_panel">
      <div class="tab_head">
        <Tabs :value="$router.currentRoute.name">
          <TabPane label="编辑认证信息" name="bianCertificate"></TabPane>
        </Tabs>
      </div>
...
import {
  getAddresslist,
  addCheckOrg,
  personalAuthenUpdate,
  goAddCheckOrg,
  personalAuthenDetail,
  checkOrgDetail,
  orgAttrs,
} from "@/api";
...
export default {
  ...
  methods: {
    ...
    handleSubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          personalAuthenUpdate(obj).then((res) => {
            console.log(res.result);
            if (res.code == 200) {
              this.$router.push({
                name: "accrInstitution",
    ...
    handleEdit() {
      ...
      let objc = {};
      objc.id = this.id;
      personalAuthenDetail(objc).then((res) => {
        console.log(res.result.areaId);
        let arr = res.result.areaId.split(",");

        if (res.result.identityFaceImg !== "") {
          this.tipg = true;
          this.formData1.identityFaceImg = res.result.identityFaceImg;
          this.avatarList2.push(res.result.identityFaceImg);
        }
        ...
        if (res.result.identityNationalEmblemImg !== "") {
          ...
          this.avatarList1.push(res.result.identityNationalEmblemImg);
        }

        if (res.result.identityNationalEmblemImg !== "") {
          this.tipg2 = true;
          this.formData1.identityInHoldImg = res.result.identityInHoldImg;
          this.avatarList3.push(res.result.identityInHoldImg);
        }
        this.formData1.principalName = res.result.principalName;
        this.formData1.principalIdentityId = res.result.principalIdentityId;
        this.formData1.principalMobile = res.result.principalMobile;
        this.formData2.trainType = res.result.trainType;
        this.formData2.contactsName = res.result.contactsName;
        this.formData2.contactsMobile = res.result.contactsMobile;
        this.formData2.storeName = res.result.storeName;
        this.formData2.orgAddress = res.result.orgAddress;
        this.formData2.pmSchoolSubject = res.result.pmSchoolSubject;
